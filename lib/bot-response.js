...
BotResponse.prototype.checkForAggressiveStatement = function() {
  ...
  BotResponse.prototype.checkForMessageAboutBrett = function() {
    ...
    var nameResponses = ["Brett's kryptonite is Chick Fil-A", "Challenge: Make Brett loose his patience. Challenge may be impossible, he taugh english to children in china you know.", "Brett can't tell the difference between a porter and a stout... total Denver beer noob"];
    for (var i=0; i<names.length; i++) {
      if (caseSensitiveUserMessage.indexOf(names[i]) > -1) {
        this.botMessageContent = nameResponses[Math.floor((Math.random()*nameResponses.length))];
      }
